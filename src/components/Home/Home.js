...
function Home(props) {
    ...
    const [memberNum, setmemberNum] = useState(sessionStorage.getItem("member_num") || null);
    ...
    const getInformation = async () => {
        try {
            ...
            const res2 = await axios.get(`${process.env.REACT_APP_SERVER_IP}/random/${todayNum}`); // /random/{question_num}
            setAnswerData(res2.data);
            const res3 = await axios.get(`${process.env.REACT_APP_SERVER_IP}/question/${todayNum}`);
            setQuestion(res3.data.question);

            if (memberNum !== null) {
                const res4 = await axios.get(`${process.env.REACT_APP_SERVER_IP}/answers/${todayNum}/${memberNum}`);
                if (res4.data.length > 0) {
                    setTodayMyA(res4.data);
                }
                const res5 = await axios.get(`${process.env.REACT_APP_SERVER_IP}/trashes/${memberNum}`);
        ...
        <div className="Home">
            <div className={props.isMobile ? "questions_mobile" : "questions"}>
                ...
                <div>
                    ...
                    <p>{question}</p>
                    <img src={VectorRight} alt="vectorRight" className="vectorRight"></img>
                </div>
            </div>
            ...
            <div id={props.isMobile ? "bottom_mobile" : "bottom"}>
                <div className={props.isMobile ? "bottom_textBox_mobile" : "bottom"}>
                    ...
                    <p>
                        매일 달라지는 질문에 답해보세요. 사소하지만 큰 기록은 분명 당신과 당신을 이어주는 고리가
                        될거예요. 3년 동안의 기록을 차곡차곡 모으고, 찬찬히 나를 만나봐요.
                    </p>
                </div>
                <img className={props.isMobile ? "bgImg_mobile" : "bgImg"} alt="bgImg" src={main}></img>
            </div>
            {loginAlert ? <LoginAlert closeLoginAlert={closeLoginAlert}></LoginAlert> : null}
