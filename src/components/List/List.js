...
function List() {
    ...
    const [answerDate, setAnswerDate] = useState(location.state === undefined ? day365 : Number(location.state.id));

    const [month, setMonth] = useState(
        location.state === undefined ? new Date().getMonth() + 1 : location.state.targetMonth,
    );
    const [date, setDate] = useState(location.state === undefined ? day365 : location.state.targetDate);
    ...
    const getQnA = useCallback(async () => {
        try {
            ...
            const answers = await axios.get(`${process.env.REACT_APP_SERVER_IP}/answers/${date}/${member_num}`);
            setAnswerAllData(answers.data);
            const questions = await axios.get(`${process.env.REACT_APP_SERVER_IP}/question/calendars/${date}`);
    ...
    function goTrash() {
        ...
        try {
            axios.patch(`${process.env.REACT_APP_SERVER_IP}/answers/trashes`, {
                ...
                delete_date: new Date(+new Date() + 3240 * 10000).toISOString().split("T")[0], //오늘날짜로, date타입
                member_num: member,
